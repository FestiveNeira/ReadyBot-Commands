...
module.exports = {
    ...
    execute(message, bot) {
        ...
        var words = fs.readFileSync('words.json', 'utf8');
        var wordlist = words.substring(1, words.length - 1).split(" ");
        var word = wordlist[Math.floor(Math.random() * wordlist.length)];

        if (!nomsgs) {
            if (msg.includes(word)) {
                message.channel.send("You said my secret word " + `<@${message.author.id}>` + "!");
            }
            if (red && (msg.includes("good morn") || msg.includes("gm"))) {
            ...
            if ((msg.includes("goodnight") || msg.includes("gn") || msg.includes("good night") || msg.includes("for the night") || msg.includes("going to bed")) && (time.getHours() >= 19 || time.getHours() <= 4)) {
